...
향후에는 추가 생체 신호 또는 시계열 정보 결합을 통해 고혈당 영역 예측 성능 개선을 목표로 할 수 있습니다.
...
본 프로젝트에서는 **SG_PLUS_META 실험**을 기반으로 모델 예측 결과에 대한 해석 가능성을 확보하기 위해 **SHAP(Global explanation)** 과 **LIME(Local explanation)** 분석을 수행하였습니다.
...
- 본 프로젝트는 **SG 기반 비침습 혈당 예측 가능성**을 실험적으로 검증하였으며, 설명가능성(SHAP/LIME), 임상 지표(MARD, CEGA), 교차검증을 포함한 **재현 가능하고 해석 가능한 머신러닝 시스템**을 제시하였습니다.

- 향후 연구에서는:
  - 추가 생체 신호(예: HRV, 연속 측정 데이터)
  - 시계열 모델링
  - 개인별 맞춤 모델링을 통해 **고혈당 영역 예측 성능 및 임상 활용성**을 더욱 향상시킬 수 있을 것으로 기대됩니다.
