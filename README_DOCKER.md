# 🐳 SG → BG 예측 프로젝트 Docker 실행 가이드(v1.1)
...
| Version | Description |
|-------|-------------|
| v1.0 | Base SG → BG prediction pipeline |
| v1.1 | XGBoost / CatBoost 추가, Weighted Ensemble 적용 |


## 🧱 Docker 이미지 빌드

🔹 (선택1) Docker Hub에서 이미지를 직접 다운로드 합니다.

```bash
docker pull eunzy213/sg-bg:v1.1
docker run --rm eunzy213/sg-bg:v1.1
```

🔹 (선택2) 프로젝트 루트 디렉토리에서 아래 명령을 실행합니다.
...
## 🤖 CLI 기반 추론 실행
🔹학습된 모델을 이용해 **새로운 입력값**으로 `BG`를 **예측**할 수 있습니다.
...
🔹프로젝트 전체 설명 및 구조는 👉 `README.md`, `System Architecture.md` 파일을 참고하세요.
