...
- ❌ Stacking Ensemble 제외 이유
  - 기본 모델들이 예측 패턴이 유사하여 Stacking을 적용하더라도 **예상되는 성능 향상폭이 제한적**이며, 메타 모델 학습 단계가 추가됨에 따라 **과적합 위험과 복잡도가 증가**함
---
...
🔹 **Gradient Boosting 계열(XGBoost, CatBoost, LightGBM)**은 기존 단일 회귀 및 RandomForest 대비 **전반적으로 성능 개선**을 보였습니다.

🔹 특히 CatBoost와 XGBoost는 **R² 향상, MAE / MARD 감소, CEGA A+B 영역 비율 증가** 측면에서 **가장 안정적인 성능**을 보였습니다.

🔹 **Weighted Ensemble**은 단일 최상위 모델 대비 큰 폭의 성능 향상은 없었으나, **예측 분산 감소 및 전반적인 안정성 측면**에서 의미 있는 결과를 보였습니다.

🔹 본 버전은 **모델 성능 고도화 + 해석 가능성 확보**를 동시에 달성한 단계로 평가됩니다.
